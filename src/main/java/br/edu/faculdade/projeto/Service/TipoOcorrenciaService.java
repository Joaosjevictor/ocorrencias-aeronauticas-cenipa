...
import br.edu.faculdade.projeto.model.TipoOcorrencia;
...
public class TipoOcorrenciaService {
    ...
    public void processarArquivoTipo(String caminhoArquivo) {
        ...
        try (org.hibernate.Session session = HibernateUtil.getSessionFactory().openSession();
            ...
            while ((linha = reader.readNext()) != null) {
                ...
                if (ocorrencia != null) {
                    ...
                    tipo.setTipo(tratarDadoConforme(linha[1])); 
                    tipo.setOcorrencia(ocorrencia);

                    session.persist(tipo);
                    count++;
                    
                    if (count % 500 == 0) {
                        System.out.println("Já salvamos " + count + " tipos de ocorrência...");
                    }
                }
            }

            session.getTransaction().commit();
            System.out.println("Importação de Tipos concluída com Sucesso! Total inserido: " + count);

        } catch (Exception e) {
            System.err.println("Erro ao ler o CSV de Tipos: " + e.getMessage());
            e.printStackTrace();
        }
    }
}